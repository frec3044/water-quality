...
Provide the link to your assignment GitHub repository here:

# Using Application Programming Interfaces (API)

Many data sources have an application programming interface (API), a clearly defined set of communication methods for accessing data on a remote server. The USGS has an API that we will use.
...
You will see that it returns a tab-delimited data table, according to the USGS website. This API call specifically requests a subset of data from the USGS water quality dataset. To see what subset it is asking for, we need to break down the API into its components.
...
We can build this URL from variables by concatenating strings and variables into a single string. For example, we can create the following variables:
...
To read data from a single request, we can read it directly.
...
Now, we need to remove the first row. `slice` is a function that selects rows based on their row numbers. The `-1` value removes the first row.
...
Combine all cleaning steps above into a single set of piped commands that creates a data frame named `stream_data`. Don't include `read_delim()` in your set of piped commands; instead, use the `df` output by `read_delim()` as the starting point.
...
The first step is to create a function that you will repeatedly use to perform the task. Functions can take input arguments and generate output. For example, here is a function that adds two numbers together. You can learn more about functions [here](https://r4ds.hadley.nz/functions.html#vector-functions)
...
get_USGS_temp_data <- function(site_no){
  ...
  if(ncol(df) < 5){ 
    ...
    #THIS IS NEEDED BECAUSE SOME SITES ONLY HAVE ONE TEMPERATURE MEASUREMENT
    stream_data <- df |> 
      rename(temperature = 5) |>
      ADD_YOUR_CODE_FROM_QUESTION_1_THAT_CLEANS_THE_DATA
  } else {
    #BUT OTHERS HAVE TWO TEMPERATURE MEASUREMENTS (TWO DIFFERENT SENSORS)
    stream_data <- df |> 
      rename(temperature = 7) |>
      ADD_YOUR_CODE_FROM_QUESTION_1_THAT_CLEANS_THE_DATA
  }
...
The class of the output is a list. That is because the `map` function automatically combines the output from the different function calls into a list. There are other functions in the `map_` family that combine the output in different ways. For example, the `map_dbl` function returns a vector of numeric values and is more appropriate for this simple example.
...
The `map_dfr()` is particularly powerful for working with data frames because it appends each data frame generated by each call to the function, row-wise. We use it to apply our function to download and clean the USGS data over multiple sites. This example uses two site codes and passes them to the function `get_USGS_temp_data()` you created above.
...
Now we can plot the data returned by the `map_dfr` call.
...
The code for answer 5 is provided.  If two lines are shown, your answer is correct.  If two lines are not shown, you must modify the code above to produce two lines.

```{r}
ggplot(temperature_data, aes(x = datetime, y = temperature, color = site_no)) +
  geom_point() +
  labs(x = "Time", y = "Water temperature (°C)")
```
...
During this portion of the module, you will download, clean, and generate plots to further analyze the data, answering questions as you go.

Watershed managers need to know how often a risk occurs in a watershed so they can take action. For financially strapped governments, priority is given to problems with the highest probability of occurring and the most severe impacts. "Blue-baby syndrome" is a condition that leads to infant mortality. It is caused by the ingestion of nitrate in drinking water, which subsequently binds to oxygen sites on hemoglobin in the blood of the infant. This impairs oxygen circulation in the bloodstream, causing the baby to turn blue. Society would like to avoid this outcome.

**Question 7:**

Modify the functions we created for downloading and cleaning temperature data to download and clean nitrate data.  Download data for the period from `2011-01-01` to the current date. The variable code for nitrate is `cb_99133`.
...
Download and clean the data from `site_no = "05464420"` using your function from question 7.  Use the function `head()` to show your data.
...
Create a plot using the data from Question 8, with datetime on the x-axis and nitrate on the y-axis (this is a time-series plot of nitrate). Then use the `geom_hline()` function to add a horizontal line at the EPA concentration (the `epa_limit` variable that you defined above)
...
Has the focal site exceeded the EPA limit?
...
A JSON file is a common format for requesting data from a remote database. JSON stands for JavaScript Object Notation. You can learn more about JSON [here](https://r4ds.hadley.nz/rectangling.html#json).  You will need to install the `jsonlite` package and use the `read_json()`  function to read in the metadata (you can call this `df_site_list`). Be sure to use the argument `simplifyVector = TRUE` to convert the JSON to a data frame.

**Question 13:**

Your answer to this question will be a mix of code chunks, plots, and text. It should include

-   Read in and prepare USGS data for analysis. Use functions introduced in this module when appropriate. Remember that you have already created the function to download and clean the data, so just copy and paste the function into this assignment. You have also learned about the `map` function to iterate over sites. 
-   Calculate the number of days per year that each site exceeds the EPA limit (you have already done this for a single site)
-   Plot the exceedance data that includes all sites (e.g., sites should be separate lines on a single plot)
...
Remember to render your document to HTML and comment+push your code and the rendered HTML to GitHub. Your GitHub repository should have multiple commits with informative commit messages.
